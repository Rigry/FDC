...
*/
			...
			enable = Start and not phase;
					/*Start and not rerun.isCount() /*and not service.pressure_is_normal() and not delay.isCount()
...
//		contactor.stop();
	...
	void alarm() {
		...
		if(not Start or (adc.is_error())/*or timer_stop.done()) or not contactor.is_on() /*or service.pressure_is_normal()
