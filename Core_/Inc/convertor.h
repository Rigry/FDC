...
public:
	...
	{rerun.time_set = 0; delay.time_set = 0; rotation.time_set = 0;}
	...
	void operator() (){
		...
		service.outData.error.overheat_c = not TD_DM;
...
//		service.outData.error.voltage_board_low = (service.outData.voltage_board <= 18);
		...
		service.outData.error.HV = service.outData.high_voltage >= 1000;
...
if(motor == ASYNCHRON) {
			...
			if (service.outData.high_voltage > 400 and service.outData.high_voltage < 540) {
				...
				min_ARR = (div_f / ((U_phase_max) * 5)) * 26; // 5/22 = 50/220
			} else
				min_ARR = 2000;
			...
			if(service.outData.high_voltage > 400 and service.outData.high_voltage < 540) {
				U_phase_max = ((((service.outData.high_voltage / 20) * 990) / 141) * 115) / 100;
				min_ARR = (div_f / ( (U_phase_max) * 70)) * 55; // 70/53 = 280/212
			} else
				min_ARR = 356;
}
...
if(motor == ASYNCHRON) {
			if (service.outData.high_voltage > 300 and service.outData.high_voltage < 540) {
				U_phase_max = ((((service.outData.high_voltage / 20) * 990) / 141) * 115) / 100;
				min_ARR = (div_f / ((U_phase_max) * 5)) * 26; // 70/53 = 50/212
			} else
				min_ARR = 2000;
...
if(motor == ASYNCHRON) {
			...
			if(TIM3->ARR <= 6500 and not rotation_) {
				rotation.start(2000);
				rotation_ = true;
			}
...
//		TIM3->ARR = (div_f / (frequency)) * 10 - 1;
	...
	void alarm() {
		if(not Start or not contactor.is_on() /*or service.pressure_is_normal()*/
		...
		{
//			if(service.pressure_is_normal()) {delay.start(90'000); error = 0;}
			stop();
		}
		if (abs(adc.value(PS) - adc.offset_I_S) / 21 > 20 and not service.outData.error.current_S) {
			error++;
			stop();
			service.outData.error.current_S = true;
			rerun.start(3500);
		}
		if (abs(adc.value(phase_A) - adc.offset_I_A) / 21 > max_current and not service.outData.error.current_A) {
			error++;
			stop();
			service.outData.error.current_A = true;
			rerun.start(3500);
		}
		if (abs(adc.value(phase_C) - adc.offset_I_C) / 21 > max_current and not service.outData.error.current_C) {
			error++;
			stop();
